...
public class ApprovalSSCController extends HttpServlet {
    ...
    protected void doGet(
        ...
        int total = pengajuanDAO.count(null, "MENUNGGU_SSC", keyword);

        request.setAttribute("pendingList", pengajuanDAO.search(null, "MENUNGGU_SSC", keyword, page, PAGE_SIZE));
        request.setAttribute("total", total);
        request.setAttribute("page", page);
        request.setAttribute("totalPages", (int) Math.ceil((double) total / PAGE_SIZE));

        request.getRequestDispatcher("/WEB-INF/views/approval/ssc.jsp").forward(request, response);
    }
